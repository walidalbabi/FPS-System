...
public class MeleeDamageTrigger : MeleeWeaponComponent
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        NetworkContext.DrawSphere(other.transform.position, _collider.radius, Color.green, 10f);
        RaycastSphere(other.transform.position);
        ServerRaycastSphere(other.transform.position);
    }

    private void RaycastSphere(Vector3 pos)
    {
        var hits = GameManager.instance.networkContext.FishNetSphereRaycast(pos, _collider.radius, _collLayer);
        {
