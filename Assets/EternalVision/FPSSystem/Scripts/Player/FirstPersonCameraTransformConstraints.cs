    ...
    public class FirstPersonCameraTransformConstraints : MonoBehaviour
    {
        ...
        void UpdatePositionConstraints()
        {
            ...
            while (itr < m_PositionConstraints.Count)
            {
                ...
                constrainedPosition = Vector3.Lerp(constrainedPosition, targetPosition, m_PositionConstraints[itr].constraint.positionStrength * EaseInOutQuadratic(m_PositionConstraints[itr].blend));
        ...
        void UpdateRotationConstraints()
        {
            ...
            while (itr < m_RotationConstraints.Count)
            {
                ...
                // Lerp based on strength and blend
                constrainedRotation = Quaternion.Lerp(constrainedRotation, targetRotation, m_RotationConstraints[itr].constraint.rotationStrength * EaseInOutQuadratic(m_RotationConstraints[itr].blend));
        ...
        public float EaseInOutQuadratic(float x)
        {
            x = Mathf.Clamp01(x);
            if (x < 0.5f)
            {
